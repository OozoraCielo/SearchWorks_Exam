<script>
    ...
    async function handleSubmit() {
		...
		if (!name || !location || !address || !price || !size || !type || !description || !features || !bedrooms || !bathrooms || !garage || !plot || !year || !pic || !room_pics) {
            error = true;
			return;
		}
        sending = true;
			...
			try {
				await addNewProperty(name, location, address, price, size, type, description, features, bedrooms, bathrooms, garage, plot, year, pic, room_pics);
                close();
                sending = false;
			} catch (err) {
				console.log(err);
